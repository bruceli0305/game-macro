...
运行时组件是Game Macro的核心支撑系统，负责在程序运行期间提供基础功能服务、状态管理、数据处理和系统协调。系统包含计数器管理、BUFF引擎、工作池控制、线程管理、工具类函数等核心模块，为整个应用提供稳定可靠的运行时环境。
...
```autohotkey
; 显示通知消息
Notify(message, title := "Game Macro", timeout := 3000, icon := "info") {
    try {
        ; 格式化消息
...
UI_MinimizeToTray() {
    try {
        if (UI.Main) {
            ...
            ; 显示托盘提示
            TrayTip("Game Macro", "程序已最小化到托盘", 2000, 1)
