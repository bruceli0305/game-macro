...
主程序文件 `Main.ahk` 是整个游戏宏工具的核心入口，负责系统的初始化、模块加载、权限检查和整体协调。它是整个应用程序的生命周期管理器。
...
主程序框架作为整个游戏宏工具的核心，承担着系统初始化、模块协调、生命周期管理等关键职责。其设计充分体现了模块化、事件驱动、性能优先的设计理念，为整个应用提供了稳定可靠的基础架构。
