...
UI_Page_Config_Build() {
    ...
    UI.SkillLV := UI.Main.Add("ListView", "x12 y12 w100 h100 +Grid +AltSubmit"
        , [T("col.skill.id", "ID"), T("col.skill.name", "技能名"), T("col.skill.key", "键位"), T("col.skill.x", "X"), T(
            "col.skill.y", "Y"), T("col.skill.color", "颜色"), T("col.skill.tol", "容差")])
    ...
    UI.PointLV := UI.Main.Add("ListView", "x12 y12 w100 h100 +Grid +AltSubmit"
        , [T("col.point.id", "ID"), T("col.point.name", "名称"), T("col.point.x", "X"), T("col.point.y", "Y"), T(
            "col.point.color", "颜色"), T("col.point.tol", "容差")])
...
UI_Page_Config_SwitchProfile(name) {
    ...
    ; 新增：选中拾色热键
    try {
        k := App["ProfileData"].PickConfirmKey
